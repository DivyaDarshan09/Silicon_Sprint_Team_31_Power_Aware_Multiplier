...
//-------------------------------------------
parameter N = 16;
parameter BATCH_SIZE = 256;  // number of parallel multiplications
...
logic signed [2*N-1:0] P_array [0:BATCH_SIZE-1];
    ...
    for (x_idx = -32767; x_idx <= 32767; x_idx = x_idx + BATCH_SIZE) begin
        for (y_idx = -32767; y_idx <= 32767; y_idx = y_idx + BATCH_SIZE) begin
            ...
            for (batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx = batch_idx + 1) begin
                ...
                // Progress display
                //-------------------------------------------
                if (tests_done % 1000000 == 0) begin
                    $display("Progress: %0d tests completed", tests_done);
                end
